...
$$
\begin{aligned}
\text{vocalizations}_{ij} & = \hat{\beta}_{0} + \hat{\beta}_{1}\text{alien vs. manq}_{ij} + \hat{\beta}_{2}\text{shape vs. manq}_{ij} + \hat{\beta}_{3}\text{human vs. manq}_{ij} + e_{ij} \\
\end{aligned}
$$
:::
:::
...
The entity sniffed had a significant effect on the number of vocalizations by sniffer dogs, `r report_ez_aov(sniff_aov, es_type = "Omega2_partial")`.
...
- $\hat{u}_{0j}$ represents the difference in vocalizations for a particular dog from the overall mean number of vocalizations
