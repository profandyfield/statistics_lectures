---
title: "workflow"
format: revealjs
---

##  `r rstudio(scale = 0.6)` project files 

![](../shared_media/images/space_pirate_trans.png){.absolute top=0 right=0 height="200"}


A file created by `r rstudio(scale = 0.4)` with the extension [.Rproj]{.alt}

- Stores information about the containing folder
- Restores the previous state of the project (i.e. what documents/tabs were open)

::: fragment

- Opening a project file sets the working directory to the folder containing the project file
  - You can use relative file paths
  - Outside of Posit cloud you can share the project folder and it will work on any machine/operating system you care to use
 
:::

::: fragment
::: {.callout-tip icon = false}
## `r robot()` : Have a go!

- Use project files!
- Posit cloud automatically uses them!
- Create an `r rstudio(scale = 0.4)` project called `my_adventr` on [RStudio cloud]{.alt}

:::
:::


## Creating a project


![](media/rs_cloud_new_project.png){.absolute top=150}

##

![](media/rs_cloud_deploy.png){fig-align="center"}

##

![](media/rs_cloud_name_project.png){fig-align="center"}

## Get organized!

![](media/dsr2_fig_04_19_project_structure.png){fig-align="center" height=600}

## Try it!

Within your `r rstudio(scale = 0.4)` project called `my_adventr` create folders called

- `data`
- `quarto`
  
::: fragment

Copy the following files from canvas

- `eddiefy.csv` to the `data` folder

:::



![](../shared_media/images/space_pirate_trans.png){.absolute top=0 right=0 height="200"}

## Customizing `r rstudio(scale = 0.6)` 

### Use the native pipe (`|>`)

![](media/use_native_pipe.png){fig-align="center" height=500}

## Colour scheme 


:::: columns
::: {.column width="50%"}

::: {.callout-tip icon = false}
## `r robot()` : Have a go!

- Windows
  - `Tools > Options`
- MacOS
  - `Tools > Global Options`
  - `Tools > Project options`

:::

![](../shared_media/images/space_pirate_trans.png){fig-align="center" height=300}

:::

::: {.column width="50%"}
![](media/rstudio_panes.png)
:::
::::


::: notes
Image of the dialog box for editing the colour scheme of `r rstudio(scale = 0.4)`
:::

## Pane locations

:::: columns
::: {.column width="50%"}
###  Have a go!

![](../shared_media/images/space_pirate_trans.png){height="500"}
:::

::: {.column width="50%"}
![](media/rstudio_panes_2.png){height="500"}
:::
::::

::: notes
Image of the dialog box for editing the location of panes in RStudio
:::

##

![](media/dsr2_fig_04_09_rstudio_overview.png)

::: notes
Image of the RStudio interface
:::



## Have a go: create a report

![](../shared_media/images/space_pirate_trans.png){.absolute top=0 right=0 height="200"}

- Test the hypothesis that Iron Maiden songs have got longer over the years.


```{=html}
<iframe width="780" height="600" src="./iron_maiden.html" title="A report about Iron Maiden"></iframe>
```




## The process of [E.V.I.L.]{.ong_bold}

![](media/evil_process.png){fig-align="center"}


## The process of [E.V.I.L.]{.ong_bold}

```{=html}
<iframe width="780" height="700" src="./iron_maiden.html" title="A report about Iron Maiden"></iframe>
```




