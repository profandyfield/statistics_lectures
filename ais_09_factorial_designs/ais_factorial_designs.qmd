...
```{r}
xbox_wald <- test_wald(incpt_lm, game_lm, console_lm, xbox_lm)
display(xbox_wald, footer = "")
```
...
The type of game significantly moderated the effect of the games console on injury severity, `r report_lrt(xbox_wald, row = 4)`.
...
xbox_console_2 + 
  annotate("segment", x = 1, xend = 30, y = console_means$Mean[1], yend = console_means$Mean[1], linewidth = 1, colour = viridis_4[1]) +
  annotate("segment", x = 41, xend = 70, y = console_means$Mean[2], yend = console_means$Mean[2], linewidth = 1, colour = viridis_4[2])
```
...
xbox_int + 
  scale_colour_manual(values = c(viridis_4[2], viridis_4[2], viridis_4[4], viridis_4[4])) +
  annotate("segment", x = 1, xend = 30, y = game_means$Mean[2], yend = game_means$Mean[2], linewidth = 1, colour = viridis_4[2]) +
  annotate("segment", x = 41, xend = 70, y = game_means$Mean[1], yend = game_means$Mean[1], linewidth = 1, colour = viridis_4[4])
```
...
::: notes
what does the b = 3.8 mean? (only for the coding scheme I have used)
:::
...
int_plot <- ggplot2::ggplot(xbox_tib, aes(x = console, y = injury, colour = game)) +
  ...
  theme_minimal(base_size = 18) +
  annotate("text", x = 1, y = int_means$Mean[2] + 1, label = sprintf("%.2f", int_means$Mean[2]), linewidth= 4, colour = grey) + 
  annotate("text", x = 1, y = int_means$Mean[1] - 1, label = sprintf("%.2f", int_means$Mean[1]), linewidth= 4, colour = grey) +
  annotate("text", x = 2, y = int_means$Mean[4] + 1, label = sprintf("%.2f", int_means$Mean[4]), linewidth= 4, colour = grey) + 
  annotate("text", x = 2, y = int_means$Mean[3] - 1, label = sprintf("%.2f", int_means$Mean[3]), linewidth= 4, colour = grey)
```
...
```{r}
xbox_se1 <- estimate_contrasts(model = xbox_lm,
                   contrast = "game",
                   by = "console",
                   comparison = "joint",
                   p_adjust = "bonferroni")
...
```{r}
xbox_se2 <- estimate_contrasts(model = xbox_lm,
                   contrast = "console",
                   by = "game",
                   comparison = "joint",
                   p_adjust = "bonferroni")
...
```{r}
alcon_tbl <- tibble::tibble(
  `Alcohol group` = c("Placebo", "Low dose", "High dose"),
  `Contrast 1 (Alcohol vs. placebo)` = c("-2/3", "1/3", "1/3"),
  `Contrast 2 (High vs. Low dose)` = c("0", "-1/2", "1/2")
)
...
```{r}
ftcon_tbl <- tibble::tibble(
  Facetype = c("Unattractive", "Attractive"),
  Contrast = c(-1/2, 1/2)
)
...
```{r}
goggles_wald <- test_wald(incpt_lm, face_lm, alcohol_lm, goggles_lm)
display(goggles_wald, footer = "")
```
...
The dose of alcohol significantly moderated the effect of the type of face on attractiveness ratings, `r report_lrt(goggles_wald, row = 4)`.
